...
import type { OverviewData, CostBreakdown } from '@/lib/parseClaudeData'
...
function OverviewContent({ data, breakdown }: { data: OverviewData; breakdown: CostBreakdown }) {
  ...
  const opusEntry = modelMixData.find(m => m.model === 'claude-opus-4-8')
  const haikuEntry = modelMixData.find(m => m.model === 'claude-haiku-4-5')
  const opusPct = opusEntry ? ((opusEntry.tokens / totalModelTokens) * 100).toFixed(1) : '0'
  const haikuPct = haikuEntry ? ((haikuEntry.tokens / totalModelTokens) * 100).toFixed(1) : '0'
...
export default function OverviewPage() {
  const { overview, costBreakdown } = useDataContext()
  ...
  return (
    <div className="space-y-6">
      ...
      <OverviewContent data={overview} breakdown={costBreakdown} />
    </div>
  )
}
